...
from ..lib.resample import MAX_POSITIONS_PER_CALL, chunk, resample
...
async def elevation_profile(
    ...
    *,
    ...
    note = (
        f"Převýšení je součet rozdílů mezi {len(known)} vzorky na {round(length / 1000, 1)} km. "
    ...
    return ElevationProfileResult(
        ...
        descent_m=int(round(descent)) if descent is not None else None,
